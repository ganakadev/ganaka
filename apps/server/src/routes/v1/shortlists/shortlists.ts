...
import { shortlistSchema, v1_schemas } from "@ganaka/schemas";
...
function findPersistentInstruments(
  ...
  const nonEmptySnapshots = snapshots.filter((snapshot) => {
    const entries = snapshot.entries as z.infer<typeof shortlistSchema>[] | null;
...
const shortlistsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/", async (request, reply) => {
    try {
      ...
      if (shortlists.length === 0 || shortlists?.[0]?.entries === null) {
        return sendResponse<
      ...
      const shortlistEntries = shortlistFromDb.entries as z.infer<typeof shortlistSchema>[];
  ...
  fastify.get("/scrap", async (request, reply) => {
    const validationResult = validateRequest(
      request.query,
      reply,
      v1_schemas.v1_shortlists_schemas.getShortlistsScrap.query,
      "query"
    );
    if (!validationResult) {
      return;
