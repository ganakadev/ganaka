...
test.describe("GET /v1/groww/credentials", () => {
  ...
  test("should return 401 when invalid developer token is provided", async ({ tracker }) => {
    const response = await authenticatedGet("/v1/groww/credentials", "invalid-token-12345", {
      validateStatus: () => true,
    });
...
test.describe("PUT /v1/groww/credentials", () => {
  ...
  test("should return 401 when invalid developer token is provided", async ({ tracker }) => {
    const creds = createValidGrowwCredentials();
    const response = await authenticatedPut("/v1/groww/credentials", "invalid-token-12345", creds, {
      validateStatus: () => true,
    });
...
test.describe("DELETE /v1/groww/credentials", () => {
  ...
  test("should return 401 when invalid developer token is provided", async ({ tracker }) => {
    const response = await authenticatedDelete("/v1/groww/credentials", "invalid-token-12345", {
      validateStatus: () => true,
    });
